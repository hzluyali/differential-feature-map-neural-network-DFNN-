...
#-----------------------第三层----------------------------
    ...
    with tf.variable_scope("layer3-conv2"):
        conv32_weights=weight_variable([3,3,128,128])
        conv32_bias=bias_variable([128])
        conv32=tf.nn.conv2d(se31,conv32_weights,strides=[1,1,1,1],padding='SAME')
        relu32=tf.nn.relu(conv32+conv32_bias)
 
    with tf.name_scope("layer3-pool3"):
        pool33=tf.nn.max_pool(relu32,ksize=[1,2,2,1],strides=[1,2,2,1],padding='VALID')
...
#-----------------------第四层----------------------------
    ...
    with tf.variable_scope("layer4-conv1"):
        ...
        conv41=tf.nn.conv2d(pool33,conv41_weights,strides=[1,1,1,1],padding='SAME')
        relu41=tf.nn.relu(conv41+conv41_bias)
        df41=DF_block(relu41)
        se41=SE_block(df41,ratio=4)

    with tf.variable_scope("layer4-conv2"):
        conv42_weights=weight_variable([3,3,256,256])
        conv42_bias=bias_variable([256])
        conv42=tf.nn.conv2d(se41,conv42_weights,strides=[1,1,1,1],padding='SAME')
        relu42=tf.nn.relu(conv42+conv42_bias)
 
    with tf.name_scope("layer4-pool3"):
        pool43=tf.nn.max_pool(relu42,ksize=[1,2,2,1],strides=[1,2,2,1],padding='VALID')
...
#-----------------------第五层----------------------------
    ...
    with tf.variable_scope("layer5-conv1"):
        ...
        conv51=tf.nn.conv2d(pool43,conv51_weights,strides=[1,1,1,1],padding='SAME')
        relu51=tf.nn.relu(conv51+conv51_bias)

        res51_weights=weight_variable([1,1,128,512])
        res51_bias=bias_variable([512])
        res51=tf.nn.conv2d(pool33,res51_weights,strides=[1,1,1,1],padding='SAME')
